...
fill(noise((x+xmd+(frameCount*nvf)-10*px),y*nvf)*255, noise((x+xmd+(frameCount*nvf)-10*px),y*nvf)*255, noise((x+xmd+(frameCount*nvf)-10*px),y*nvf)*255);
rect((x+xmd+(frameCount)-10)*px, y*py, px, py);
}
}
}
if ((x+xmd+(frameCount)-10)*px<xmnd) {
println("Initalizing: " + round((x+xmd+(frameCount)-10)/xmnd*100*px) + "%" + " (" + round((x+xmd+(frameCount)-10)/xmnd*width) + " pixels)");
} else if ((x+xmd+(frameCount)-10)*px===xmnd) {
println("Initializtion Complete" + " (" + round((x+xmd+(frameCount)-10)/xmnd*width) + " pixels)");
}
};
