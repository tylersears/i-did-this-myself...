...
var s1 = function(x, y, f, c) {
    ...
    translate(x, y);
    var con = w / f * 3;
    if (c > 0 && c <= 1) {rect(con * 0, con * -1, w / f, h / f);}
    if (c > 1 && c <= 2) {rect(con * 1, con * -1, w / f, h / f);}
    if (c > 2 && c <= 3) {rect(con * 1, con * 0, w / f, h / f);}
    if (c > 3 && c <= 4) {rect(con * 1, con * 1, w / f, h / f);}
    if (c > 4 && c <= 5) {rect(con * 0, con * 1, w / f, h / f);}
    if (c > 5 && c <= 6) {rect(con * -1, con * 1, w / f, h / f);}
    if (c > 6 && c <= 7) {rect(con * -1, con * 0, w / f, h / f);}
    if (c > 7 && c <= 8) {rect(con * -1, con * -1, w / f, h / f);}
    popMatrix();
};
...
var total = false;
var block = 1;
if (total) {block = 37449;}
var drawserpinski = function(block){
if (block > 0 && block <= 1 || total) {
    b1(w/2, h/2);
} if (block > 1 || total) {
